...
// Import sensor and action enum values for cleaner switch statements
using namespace Core::Genetics;  // AGE, LOC_X, LOC_Y, etc.
...
void printSensorsActions() {
  ...
  for (i = 0; i < Sensor::NUM_SENSES; ++i) {
    fmt::print("  {}\n", Utils::sensorName((Sensor)i));
  }
  fmt::print("Actions:\n");
  for (i = 0; i < Action::NUM_ACTIONS; ++i) {
    fmt::print("  {}\n", Utils::actionName((Action)i));
  }
  fmt::print("\n");
}
...
void Individual::printIGraphEdgeList() const {
  for (const auto& conn : nnet.connections) {
    if (conn.sourceType == Genetics::SENSOR) {
      fmt::print("{}", Utils::sensorShortName((Genetics::Sensor)(conn.sourceNum)));
    } else {
    ...
    if (conn.sinkType == Genetics::ACTION) {
      fmt::print("{}", Utils::actionShortName((Genetics::Action)(conn.sinkNum)));
    } else {
...
void displaySensorActionReferenceCounts() {
  ...
  for (unsigned i = 0; i < sensorCounts.size(); ++i) {
    if (sensorCounts[i] > 0) {
      fmt::print("  {} - {}\n", sensorCounts[i], Utils::sensorName((Sensor)i));
    }
  }
  fmt::print("Actions in use:\n");
  for (unsigned i = 0; i < actionCounts.size(); ++i) {
    if (actionCounts[i] > 0) {
      fmt::print("  {} - {}\n", actionCounts[i], Utils::actionName((Action)i));
    }
  }
}
