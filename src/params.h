...
//    2. Add a member and its default value to privParams in
...
// modified by UI events by ParamManager::uiMonitor(). The main simulator thread
...
struct Params {
  ...
  uint16_t gridSize_X;              // 2..0x10000
  uint16_t gridSize_Y;              // 2..0x10000
  ...
  unsigned parameterChangeGenerationNumber;  // the most recent generation
                                             // number that an automatic
                                             // parameter change occured at
};
