...
class Grid {
 public:
  ...
  bool isInBounds(Coordinate location) const {
    return location.x >= 0 && location.x < sizeX() && location.y >= 0 &&
           location.y < sizeY();
  }
  bool isEmptyAt(Coordinate location) const { return at(location) == EMPTY; }
  bool isBarrierAt(Coordinate location) const {
    return at(location) == BARRIER;
  }
  // Occupied means an agent is living there.
  bool isOccupiedAt(Coordinate loc) const {
    return at(loc) != EMPTY && at(loc) != BARRIER;
  }
  bool isBorder(Coordinate loc) const {
    return loc.x == 0 || loc.x == sizeX() - 1 || loc.y == 0 ||
           loc.y == sizeY() - 1;
  }
  ...
  const std::vector<Coordinate> &getBarrierLocations() const {
    return barrierLocations;
  }
  const std::vector<Coordinate> &getBarrierCenters() const {
...
extern void visitNeighborhood(Coordinate loc, float radius,
                              std::function<void(Coordinate)> f);
