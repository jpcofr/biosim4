...
protected:
  ...
  std::vector<Coordinate> collectVisitedCoords(Coordinate loc, float radius) {
    ...
    auto collector = [&visited](Coordinate c) {
      visited.push_back(c);
    };
...
TEST_F(GridVisitNeighborhoodTest, VisitNeighborhoodRadius1_4) {
  ...
  EXPECT_GE(visited1_4.size(), visited1_0.size()) 
    << "Larger radius should visit same or more cells";
}
...
TEST_F(GridVisitNeighborhoodTest, VisitNeighborhoodRadius2) {
  ...
  EXPECT_GT(visited2.size(), visited1.size()) 
    << "Radius 2.0 should visit more cells than radius 1.0";
}
...
TEST_F(GridVisitNeighborhoodTest, VisitNeighborhoodEdgeCase) {
  ...
  Coordinate edge{
    (int16_t)(parameterMngrSingleton.gridSize_X - 1),
  ...
  for (const auto& c : visited) {
    EXPECT_LT(c.x, parameterMngrSingleton.gridSize_X) 
      << "X coordinate should be within grid bounds";
    EXPECT_LT(c.y, parameterMngrSingleton.gridSize_Y) 
...
TEST_F(GridVisitNeighborhoodTest, VisitNeighborhoodPrintsCorrectly) {
  ...
  auto printLoc = [&ss](Coordinate loc) {
    ss << loc.x << ", " << loc.y << std::endl;
  };
