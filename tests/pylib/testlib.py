...
def resultsAnalysis(thisconfig, testname, results_log, adjust=False):
    ...
    if not complete:
        ...
        if adjust:
            print(
                "\n--adjust was requested but will be ignored while this "
                "test has an incomplete set of result params.\nthe script "
                "will now offer to add missing params to this test's "
                "configuration\n"
            )
            adjust = False
        u = input("press any key...")
        print(
            "\nYou can terminate script execution and manually update "
            "the test config file with the missing params, and then run "
            "this test again. Or proceed (recommended) and the missing "
            "result params will be added to this test's configuration\n"
        )
    ...
    try:
        ...
        for k, v in resdict.items():
            ...
            print("  %s%s %s%s %s%s %s" % (k, pad(k), showrp, pad(showrp, 24), v, pad(v), success))
    except Exception as e:
