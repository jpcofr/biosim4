...
# Credit: Thanks to @venzen for the initial design and implementation
# of this test framework.
...
from pylib import config, include_tests, testlib
...
\n%s initializes a test environment and simulation parameters for a %s
simulation. Always execute %s inside the '<project_root>/tests/' working
directory. To see a list of configured tests, use the --show flag:
...
Inspect './configs/%s' to see the predefined example 'quicktest'. This test's
simulation and result parameters can also be output to the console:
...
1. Copy the default config file '%s' to the 'configs/' directory and rename
the copy with the unique name of your new test:
...
Simulation parameters (from .configs/my_new_test.ini) are converted to
test-style format and stored in a new section 'my_new_test' in the
configuration file './configs/%s'

After being imported, test parameters can be edited directly in the '%s' file.

Result parameters:

Result parameters can be manually added to '%s', or they can be initialed
automatically by simply running a test simulation:

    python3 %s --test my_new_test

After completing the simulation, the script will propose to add result
parameters (using actual test result values) to the config file. These can
then be manually edited to match variations in test results.
""" % (
    _scriptname,
    _projname,
    _scriptname,
...
argp = argparse.ArgumentParser(description="Script for running %s" % _projname + " simulation tests")
argp.add_argument(
    "-a",
    "--adjust",
    action="store_true",
...
argp.add_argument("-u", "--usage", action="store_true", help="see more detailed usage instructions")
argp.add_argument(
    "-v",
    "--verbose",
    action="store_true",
...
# concatenate additional tests (with more params) stored in
# pylib/include_tests.py
TEST_DEFAULTS += include_tests.incl_list


# set US locale to ensure well-defined behavior for number formatting.
# (This comment is kept for context but the code is executed earlier)
